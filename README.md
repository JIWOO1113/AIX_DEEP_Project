...
이 프로젝트는 정해진 정답 모델을 두고 시작한 것이 아니라, 팀원이 각자 다른 모델을 맡아 직접 다뤄보며 환경음 분류에 어떤 접근이 적합한지 확인하는 방식으로 진행되었다. 그중 MLP를 담당하였다.
...
## 6-4. 담당 모델(MLP·AST)을 관통하는 관찰

MLP와 AST는 "소리의 시간 정보를 입력에서 어떻게 다루는가"라는 축에서 서로 대비된다. MLP는 시간축을 통계로 압축해 시간 정보를 버리고, AST는 spectrogram을 시간축까지 보존한다. 이 차이가 실제 분류 결과에서 어떻게 나타나는지를 두 모델로 확인할 수 있었다.

MLP에서 가장 두드러진 약점은 반복 타격음인 `jackhammer`와 `drilling`을 서로 구별하지 못한 것이었다(IV-3 참조). 두 소리는 시간에 따른 반복 리듬이 핵심 단서인데, 시간 정보를 평균으로 압축하면 이 단서가 사라진다. 시간 정보를 보존하는 AST에서는 이 혼동이 거의 나타나지 않아(FE·Full FT 모두 상호 오분류 0~1건), 해당 혼동이 모델 구조 자체보다 **입력 설계에서 비롯된 것**임을 확인할 수 있었다.

다만 입력 방식을 바꾸는 것으로 모든 오분류가 해소되지는 않았다. `siren`을 다른 class로 놓치는 혼동은 AST에서도 남았는데, 이는 시간 정보의 보존 여부와 무관하게 **소리 자체가 음향적으로 겹치는 데이터의 특성**에 가깝다. 즉 두 모델의 비교를 통해, MLP의 한계 중 어디까지가 입력 설계에서 비롯된 것이고 어디부터가 데이터 자체의 모호성인지를 구분할 수 있었다.

> (모델 전체의 종합 성능 비교와 우열 판단은 「7. 최종 비교 분석」에서 다룬다. 여기서는 담당한 두 모델을 시간 정보 보존이라는 하나의 축에서 연결해 관찰하는 데 한정한다.)
