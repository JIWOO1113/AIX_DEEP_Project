...
출력:
  - models/MLP/intermediate/: feature cache
  - models/MLP/mlp_best.pt  : best checkpoint
  - models/MLP/final/       : overall_metrics.csv, classwise_metrics.csv,
                              predictions.csv
...
FEAT_PATH = INTERMEDIATE_DIR / "mlp_features.csv"   # feature 캐시 (있으면 재사용)
...
def main():
    ...
    cw = pd.DataFrame({"model": "MLP", "class": class_names, "precision": p, "recall": r,
                       "f1": f, "support": s})
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(IMAGE_DIR / "mlp_confusion_matrix.png", dpi=150, bbox_inches="tight")
        plt.close()
    except Exception as e:
    ...
    print(f"confusion matrix 이미지 저장: {IMAGE_DIR / 'mlp_confusion_matrix.png'}")
    print("=== 완료 ===")
