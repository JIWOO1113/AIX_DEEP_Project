...
#### 평가 지표
SR을 줄이면 
물리적 정보인 소리자체의 주파수 정보가 사라진다
MFCC로 변환하면 숫자 값이 달라진다.

에너지 보존율 = 원본 wav에 있던 소리 에너지 중, 새 SR로 줄여도 보존되는 비율
(이론적 손실량을 알기 위함)

MFCC 코사인 유사도 = 원본 SR로 만든 MFCC와 새 SR로 만든 MFCC가 얼마나 비슷한가
숫자 배열인 벡터가  얼마나 같은 방향을 가리키는가.
